...
- **Hardcoded-mirror-of-an-existing-helper anti-pattern (AI-shape bucket).** If a diff hardcodes a value/format that an existing helper already produces — and *especially* if a comment says "mirrors / aligned with / same as `<func>`" — that comment is the smell: call `<func>`, don't duplicate it. Duplicated conventions drift; the copy that builds a real path/key (not just a log string) is a silent-correctness bug waiting to happen. Pre-submit recipe: for any `# mirrors/aligned with <name>` comment, `grep -rn "def <name>"` to find the source, import it, and **sweep for EVERY copy** of the literal (`grep -rn "<the-literal-pattern>"`) — a reviewer's comment on one line usually generalizes to siblings in other files. Verify the helper's output is byte-identical to the literal before swapping (same args matter — pick the args that keep behavior correct, not just the ones that compile). Adding the helper's buck dep to each consuming target is part of the fix. **Learned 2026-06-23** (D109521242, reviewer wendiyu): diff hardcoded `mvai-training-online-{id}` in 4 sites with `# Mirrors generate_mast_job_name` comments; the worst copy built the actual manifold path `tree/jobs/<name>/info.json` (a real reset-target, not a log). Fix: route all 4 through the canonical `get_mast_job_name(model_id, ai_stack)` helper so path + logs share one source of truth.
...
Before running `jf submit`, verify ALL seven:
...
2. **`Reviewers:` has real reviewers** (not empty, not yourself). Use `#project-name` for project reviewers. The diff author cannot be a reviewer — Phabricator rejects it.
   - **A `#group` reviewer MUST have ≥1 member.** Adding a group with ZERO members is as bad as no reviewer: the diff looks reviewed but can never get accepted — nobody is on the hook. (Learned 2026-06-26: D109863645 was sent to `#minimal_viable_ai`, a 0-member group; it only landed because a human happened to notice and accept it.) **Verify membership before using a group** — `meta phabricator.project members <group>` (or the project's Members page), or just check the group has people. If a relevant group is empty (or you're unsure), add a **named individual** who actually reviews this area (e.g. a recent author/owner of the touched files, or someone from the originating discussion). Prefer a named individual + the group over a group alone. A reviewer that can't act is a silent no-op, the same class as a 0-member oncall or an unwired helper.
...
7. **Unit test for new testable logic** — any new/changed function, parser, regex, classifier, or mapping ships with a `test_<name>.py` + `python_unittest` target (mirror a sibling test), covering happy path + ≥1 edge case; RUN it and confirm pass. A CLI/render run or backtest is NOT a unit test. Doc/config/trivial-wrapper only = exempt (state why). This is the HARD form of the §20 prose rule, which kept getting skipped (D109480279, 2026-06-23, 2nd occurrence) — now enforced as **diff-subagent mandatory check #5**. The "spec without enforcement = unenforced" (P-011) fix: the rule existed in prose for weeks and was still missed; only the subagent hard-check stops it.

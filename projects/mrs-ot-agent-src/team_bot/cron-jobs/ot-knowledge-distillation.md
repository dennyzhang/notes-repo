...
4. **Pattern detection — six action thresholds, only first-matching action fires per run** (cap 1 diff per run, operator review bandwidth):
   ...
   | **Expected-procedure gap (P-023):** a triage/thread reveals the corpus holds only break-fix patterns for a domain it triages, but NO canonical *expected procedure* (how the thing is correctly done/enabled) — OR a confirmed root cause turned on a documented procedure the corpus never captured | ≥1 | Draft ingestion of the canonical procedure into the corpus: a crisp summary + authoritative link (skill runbook / wiki / design doc / `file:line` source) into the relevant `known-patterns.md` / reference doc. Expected-procedures are first-class learning targets, not just incident patterns. Cite P-023. |

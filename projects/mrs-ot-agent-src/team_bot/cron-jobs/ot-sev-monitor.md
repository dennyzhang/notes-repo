...
10. After loop: persist state, update last_run_epoch, **persist `gchat_health` block from step 0.5 counters**, respond with a single-line `HEARTBEAT_OK {…}` that carries per-SEV URLs for `ot-human-attention-brief` link extraction. **Item 25 (2026-06-25): NO narration before HEARTBEAT_OK, NO run-summary block after it** — the in-thread triage posts ARE the delivery. The final response must be exactly one line:
    ...
    **Consecutive-403 escalation (from step 0.5 gate):** if `NEW_CONSEC ≥ 3`, post ONE line to spaces/AAQAVOjYc80 with `--as-meta-bot`: `🚫 GChat reads degraded ≥3 ticks — tried buck2 run; need help (consecutive=$NEW_CONSEC, last_ok=<HH:MM PT>)`. Only fires on the first tick that crosses the threshold (skip if `PRIOR_CONSEC ≥ 3`) to avoid spam.
    ...
    ```
    HEARTBEAT_OK {"sevs_processed":N,"new_in_scope":M,"sev_urls":["https://www.internalfb.com/sevmanager/view/<id1>",...],"reply_urls":["https://chat.google.com/room/AAQAVOjYc80/<thread1>",...],"gchat_health":{"403_seen":0,"refresh_attempted":0,"refresh_ok":0,"consecutive":0,"last_ok_age_min":0}}
    ```

    **Compliant shapes (item 25):**
    - SEVs triaged in-thread: one-line `HEARTBEAT_OK {...}` with URLs in JSON — GChat daemon suppresses delivery, `ot-human-attention-brief` extracts URLs from `raw_response` sqlite column.
    - No new in-scope SEVs: `HEARTBEAT_OK {"sevs_processed":0,"new_in_scope":0,"gchat_health":{...}}`
    - FORBIDDEN: any text before `HEARTBEAT_OK`; any `---` separator or `**Run summary**` block on a separate line.

    GChat-health block added 2026-05-28 per thread AOQM8O19bHE — carries 403/refresh instrumentation in the JSON blob (queryable from sqlite `raw_response`).
...
19. [2026-06-21 re-apply] **Add `\btraining\s+degrading\b` to step-3 OR clause.** Add `\btraining\s+degrading\b` (case-insensitive, word-boundary on "training") as a step-3 OR clause in both step-3 Query B title-match regex and `sev_identification._EXPLICIT_PATTERNS_DEFAULT`. Scope note: IGR/ESR-prefixed SEVs are still filtered by scope_check after admission. Source: S675887 "[IGR][ESR] combo7-2-3 model has training degrading..." not detected by step-3 regex. (Claimed 2026-06-15 L2, 2026-06-16 L1, 2026-06-18 re-apply A — ledger entries without notes execution; applying now.)

20. [2026-06-21 re-apply] **Defer candidate SEV if <5 min old with empty body.** If a candidate SEV passes scope_check (`in_scope=true`) but is <5 min old, has empty body/incident_impact, no parseable model ID, and zero GChat messages → defer: do NOT add to `diagnosed_ids`, log `triage_deferred=true, reason=too_new_empty_body`. Next tick retries with fresher content. Do NOT page on a <5-min-old empty SEV. Source: S676418 (3 min old, empty body, zero GChat at triage time). (Claimed 2026-06-15 L3, 2026-06-16 L3, 2026-06-18 re-apply B — not in notes; applying now.)

21. [2026-06-21 re-apply] **Add `\bcogwheel\b` to step-3 OR clause.** Add `\bcogwheel\b` (case-insensitive, word-boundary) as step-3 OR clause in Query B title-match regex AND `sev_identification._EXPLICIT_PATTERNS_DEFAULT`. Cogwheel is MRS conveyor infrastructure; multi-conveyor fbpkg build failures are high-priority OT events. Source: S677120 "fleet-wide cogwheel fbpkg build failure, 5 MRS conveyors blocked" — missed all step-3 patterns. (Claimed 2026-06-18 L1 as item 33 — not in notes; applying now.)

22. [2026-06-21 re-apply] **Verify correlated SEV state before causal-cite.** Before writing any SEV into a causal chain, verify current state via `meta sevmanager.sev metadata --sev=S<id> -o json | jq .state`. Mitigated ≠ active cause. If unverifiable → tag `[INFERRED]` and reduce confidence one notch. Source: validator `wrong_causality` — S677534: bot cited S677527 (ZippyDB) as In Progress but it was Mitigated; S658668: bot cited S672106 (IPNext MI350X) without status check. (Claimed 2026-06-18 L2 as item 34 — not in notes; applying now.)

23. [2026-06-21 L1] **Run summary FORBIDDEN in final response when new_in_scope=0.** When all new Query B candidates are OOS and new_in_scope=0, the final response MUST be `HEARTBEAT_OK` (optionally + JSON metrics on the SAME line) with NO run summary paragraph. A run-summary paragraph after `HEARTBEAT_OK` causes sporadic daemon delivery — daemon behavior is inconsistent; only defense is to never emit the run summary when the result is no-action. Source: ot-sev-monitor 2026-06-20T21:26:12 (delivered=delivered — run summary leaked to GChat despite HEARTBEAT_OK token being present; 5 OOS candidates, new_in_scope=0, no action needed).

24. [2026-06-23 L1] **Verify SEV owner from metadata before citing — extends item 22.** Item 22 mandates verifying SEV STATE before causal-cite. Extend this to the SEV OWNER field: before naming an owner/on-call in any triage report, extract from `meta sevmanager.sev metadata --sev=S<id> -o json | jq '.oncall_unixname // .owner_unixname'`. Do NOT infer from model history, related models, or memory. Model owner ≠ SEV owner. If the field is empty → tag `[owner unverified]`. Source: S678710 ot-sev-monitor 2026-06-22T16:30 — bot cited kmzhang (model-history inference), validator found xxzhu (actual SEV oncall).

25. [2026-06-25 L1] **No narration before HEARTBEAT_OK — even when real triage was posted in-thread.** When GChat triage replies have already been posted in-thread during the run, the cron's final response is STILL just `HEARTBEAT_OK` (optionally + JSON metrics on the SAME first token line). The in-thread posts ARE the delivery; HEARTBEAT_OK suppresses the redundant cron-response. Forbidden: any text (narration, state-update prose, "Final output:", "Lock released.", "Composing final run summary.", "All steps complete.") before `HEARTBEAT_OK`. Forbidden: emitting `HEARTBEAT_OK` mid-response followed by a run-summary block — if there is a run-summary block, it must be the ONLY content (the post-block shape). Two violations in 24h: 2026-06-24T15:35 "All steps complete. State updated (90 IDs)… Final output:" + 2026-06-25T06:30 "Lock released. Composing final run summary." — both delivered=delivered. Generalizes item 23 (which covered new_in_scope=0) to ALL runs including those with active triage.

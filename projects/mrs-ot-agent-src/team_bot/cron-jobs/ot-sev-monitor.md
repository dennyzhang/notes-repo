[ot-sev-monitor cron] Every 15 min (tightened from hourly 2026-06-07 for detection speed — operator: "get triaged as fast as possible"). Identify NEW SEVs the mrs_online_training oncall is "looped into" (tagged OR title-class-matched), cluster by shared root cause, post one notification + deep-triage diagnosis per cluster, then a multi-lens adversarial validator PANEL (step 9.d). The run lock (step 0) serializes overlapping ticks, so the heavier panel never causes double-processing — quiet ticks get true 15-min latency; busy ticks (panel still running) are skipped by the lock and resume next tick. The OMH-style `oncall.feed` query is too narrow — misses cross-team SEVs like S654315 (mvai_ifr_main publish failure).

**TEAM-SPACE GATE — surface only incidents that NEED A HUMAN (2026-05-30).** Post to the team space `spaces/AAQA2bZMw24` ONLY when the bot cannot fully handle the incident itself: either (a) triage confidence is LOW or MEDIUM (bot unsure → a human should look) — **EXCEPT a `MONITOR` verdict, which ALWAYS routes to the operator 1:1 ONLY regardless of confidence (per L74): a MONITOR means there is no human action, so MONITOR/LOW and MONITOR/MEDIUM do NOT satisfy clause (a)** — OR (b) the verdict is a PAGE requiring a named human owner to act. If the bot fully handled it at HIGH confidence with no human action needed — NO_ACTION, auto-recovered, known-transient, or a confident MONITOR with nothing for a human to do — DO NOT post to the team space: record to state + the operator 1:1 (`meta google.chat.message send --space-name=spaces/AAQAVOjYc80 --reply-in-thread=<thread | `# new-topic`>`), then respond `HEARTBEAT_OK`. Rationale: the team space is the "bot needs help" lane; confidently auto-handled incidents are noise there. (Escalations — e.g. gchat-degraded ≥3 ticks — are exempt and still post.)
...
**`notification_outcome` schema (v3, same as ot-post-monitor / ot-alert-monitor).** One of: `POSTED:<msg_resource_name>` (notification sent + threaded reply created), `OOS:<reason>` (out-of-scope; e.g. `OOS:serving_stage_T4`, `OOS:preemptive_launch`), `DEDUP:<source>` (handled by another path), `ERROR:<one-line>` (send failed). State advance with `notification_outcome` absent OR starting with `ERROR:` keeps the entry but flags it for `ot-cron-health-guard` class 6 audit (silent-drop detection).

**MEASUREMENT-SUBSTRATE WRITE — MANDATORY per triaged SEV (RESTORED 2026-06-07).** At the SAME point you advance state with the SEV's `notification_outcome`, also write one `triage_events` row (ABSOLUTE path — a relative `tools/…` does NOT resolve from the cron's runtime cwd; the 2026-06-07 "restore" used a relative path so the write silently kept failing `MISSING` and the substrate stayed unfed — same trap as `lib-url.sh` 2026-06-06): `bash ~/notes/users/dennyzhang/projects/mrs-ot-agent-src/tools/record-triage-event.sh --sev-id <S###> --cron ot-sev-monitor --signal "<one-line signal>" --confidence <0-1> --validator-outcome <confirmed|discrepancy|unavailable> --suggested-owner <unixname> --auto-tag-applied <0|1> --final-status "<verdict>" --notification-text "<the notification one-liner>" --routed-to <auto|human>` — and, WHEN this run reached a confident root-cause verdict (root-cause: `known`, especially a cited P-row / known-pattern / R-rule match), ALSO append `--root-cause-at "$(date '+%F %T')"` to stamp WHEN the root cause was identified (feeds metric #1 time-to-root-cause via `tools/time-to-root-cause.sh`; omit the flag when root-cause is `partial`/`not found` so the column stays NULL). **`--routed-to` = the autonomy metric (`tools/escalation-rate.sh`, goal: human ≤5%): pass `human` whenever the TEAM-SPACE GATE above fired (you posted to `spaces/AAQA2bZMw24` because the bot could not fully handle it); pass `auto` otherwise (handled end-to-end: NO_ACTION / auto-recovered / confident MONITOR / 1:1-only).** This is the substrate `ot-metrics-rollup` (precision/recall/lag), `ot-triage-auditor`, and the AI-handling stats READ. **It was silently dropped from this cron ~2026-05-07, so the table froze at 20 rows for a month while every reader ran on dead data (the green-but-empty organ; cheatsheet §5/contract-#5).** Deterministic write in a shared helper (§14b/§14c — alert/post monitors call the same one). Skip ONLY a genuinely out-of-scope drop (OOS:*).

**CODE-MITIGATION AUTO-FIX GATE — MANDATORY after every REAL_OT_FAILURE / UPSTREAM_INFRA verdict (2026-06-11, operator: "how to make this automated instead of waiting me to nudge").** A code-rooted or code-mitigable failure must NOT stop at a posted verdict — that is the lag the operator flagged: e.g. `generate-job-config-diff` RE-throttle was classified UPSTREAM_INFRA and left at "monitor/retry"; the in-code retry/backoff fix only happened on a manual nudge (T275529522 / D108337795). The triage ALREADY produced a `file:line` code-pointer (Files-touched / Next-actions). At verdict time, decide `code_mitigation`: is there a plausible IN-CODE resilience fix (retry/backoff/guard/timeout/fallback) in **MRS-OT-owned tooling**, EVEN IF the root cause is upstream? (P-016/P-017 split — upstream *cause* ≠ no in-code *mitigation*.)
...
- **RECONCILE-ASSERT (anti-lag enforcement):** a REAL_OT_FAILURE / UPSTREAM_INFRA verdict whose triage_events row carries NO `code_mitigation` decision (or, for UPSTREAM_INFRA, no `upstream_confirm`) is INCOMPLETE — do not finish the run without it. Mechanical step, not a prose expectation that gets skipped under triage focus.
- **NOISE GATE + CAP:** auto-file only when fix-site is MRS-OT-owned tooling AND failure is recurring OR high-confidence-code-rooted; cap ≤2 auto-fix tasks/run (excess defers — recurring failures resurface). Dedup: skip if an open `[OT auto-fix]` task already covers the same file/symptom.
...
0. **Concurrent-run guard (2026-05-27).** Before reading state, acquire run lock:
   ...
   # TTL=1800 (2026-06-07): interval dropped to 900s, but a 3-cluster panel run
   # (4 parallel lenses/cluster) can take ~10-15 min. TTL must exceed worst-case
   # run duration so a legitimately-running heavy tick isn't treated as stale and
   # double-started; 1800 clears a truly-hung lock within 2 ticks.
   LOCK_MAX_AGE=1800
   NOW=$(date +%s)
   if [ -f "$LOCKFILE" ]; then
...
3. From (B), filter to titles matching this OT signal-class regex (case-insensitive — intentionally lossy on inclusive side, deep-triage filters false positives):
   /(mvai|online[\s._-]?train|publish(?!er)|TGIF|snapshot|delta|streaming|hedwig|silvertorch|gmpp|ifu[._-]?lsr|MRS[._-]?OT|mtml|NCCL|model.age|ATS|scribe.lag|\bOT[\s._-]?jobs?|\bOT[\s._-]?train)/i

   **Regex history:** the `online[\s._-]?train` cluster was widened 2026-05-16 (was `online[._-]?train`) after S664106 "Threads Feed teacher model 2128461099 **online training** cannot get started" wasn't matched because the title used a literal SPACE separator. Without the `\s` widening, no-tag SEVs with space-separated `online training` in title fall through. `\bOT[\s._-]?jobs?` added 2026-05-29 (L69) after S669019 "Reels LSR MB9 - OOMs on OT jobs" was missed — "OT" abbreviation without "online training" spelled out fell through both Query A (untagged) and step-3 regex. `\bOT[\s._-]?train` added 2026-05-30 (L72) after S669681 "OT training runs producing NaN immediately on startup" was self-noted as a miss — "OT training" abbreviation form not covered by `\bOT[\s._-]?jobs?` or `online[\s._-]?train`.
...
4.5. **Scope check via `scope_check` capability — CRITICAL, runs BEFORE notification.** Single source of truth — see `fbcode/pe_mrs_ml/mrs_ot_agent/src/capabilities/team_lane_scope.py`. Do NOT inline regex.
   ...
   **OWED capability change (2026-06-10 Ads `aps` leak fix — fbcode, batch into weekly sync).** `team_lane_scope.is_in_mrs_org_scope`'s title-hard-exclude is missing `aps`/`aps_`/`aps-training-online` (Ads Prediction Service), so an Ads APS SEV/alert can slip the capability gate. Add those three terms to the hard-exclude list in `team_lane_scope.py` AND tighten the positive-MRS marker so a surfaced job/model must be `mvai-`-prefixed (job name `mvai-training-online-<eid>`) or present in `human-input/models.md` before `in_scope=true` — denylist-plus-allowlist, same shape as `daily-brief.md` rule 4. Add a regression case to `tests/test_team_lane_scope.py` (Ads `aps-training-online` SEV → `in_scope=false`). This file is canonical in fbcode (`.py` git-ignored in notes); the prompt-side denylist above is the interim guard until the capability lands.
...
9. For each cluster (cap 3 per run):
   ...
   **ORDERING NOTE (2026-05-16):** Run step 9.b deep triage FIRST. Only after R18 (step 9.b.v.2) and the silent-drop backstop (step 9.b.v) both pass should step 9.a notification fire. Rationale: notifying before stage classification produced user-facing noise — see S659877 (2026-05-16 07:20–07:22 PT in spaces/AAQAVOjYc80) where a T4 serving-tier IGML capacity SEV produced a 🚨 main-space notification at 07:20 and then a `[OUT-OF-SCOPE: T4 serving-stage]` retraction at 07:22. The R18 gate worked as designed but fires too late in the flow. Execute order: 9.b → 9.b.v → 9.b.v.1 → 9.b.v.2 (R18) → if T4/out-of-pipeline, drop and emit single-line `[OUT-OF-SCOPE]` only; else 9.a notification → 9.c threaded diagnosis → 9.d validator → 9.e auto-tag → 9.f state update.
   ...
   a. Send notification to spaces/AAQAVOjYc80:
      ...
      **TRIAGE-OUTPUT LINT GATE (HARD — T274815014, pre-POST forcing function).** Before sending, lint `$NOTIF_TEXT` through the deterministic triage-output gate. A non-zero exit BLOCKS the post: fix the flagged gaps in `$NOTIF_TEXT` (cite every quantitative claim with `[VERIFIED: <meta/Scuba>]`, label unverified mechanisms `[INFERRED]`, attach a resolvable SEV/alert URL, add the missing section) — or downgrade the claim — then re-lint. Loop until clean; only a clean (exit 0) verdict may post. This is the DETERMINISTIC layer; the independent validator agent + codex pass is the ADVERSARIAL layer (two-layer rigor, same as diff review).
      ```bash
   ...
   b. DEEP TRIAGE per SKILL.md "Triage Discipline":
      ...
      **i-pre.b. QE-MODEL INFRA-VS-MODEL DISAMBIGUATION (MANDATORY when SEV is on a QE / launch-candidate model).** When the SEV title or body indicates the affected model is a QE / launch-candidate (suffix `_qe`, `_lc`, `mb9_qe`, MB-launch-blocker context, etc.), the disambiguation question is: **infra-side (would also break prod baseline) or model-side (specific to this QE's code changes)?** Follow `ot-alert-monitor.md` step i-a.4 (same two-check procedure: (1) prod-baseline-parallel liveness check, (2) sibling recurring-flow check). **Verdict routing depends on outcome — model-side roots get routed to model owner, NOT MRS-OT infra. Cite the disambiguation check verbatim in evidence.** Source: operator guidance 2026-05-18.
      ...
      i-c-FALLBACK. **ACL-proof GraphQL read of the SEV war-room (2026-06-13, the "Rovert" path).** The step-0.5 wrapper only self-heals an OAuth **403**; it CANNOT fix a `META_CLI_AGENT_ROLE_ACCESS_DENIED` (`agent_role_myclaw` ACL wipe — happened fleet-wide 2026-06-10/11 on a security rollout, can recur). When `gchat_read_with_recovery` returns that error string (grep stderr for `META_CLI_AGENT_ROLE_ACCESS_DENIED` or `ABAC_AGENT_ROLE_DENIED`), DO NOT mark UNAVAILABLE yet — fall back to the GraphQL SEV-chat edge, which goes through the data graph and is NOT gated by that ACL. It reads by SEV **number** (already in hand — no space-URL parse needed). Use the SEV's bare integer number (strip any leading `S` from the SEV id, e.g. `S674219` → `674219`):
      ```bash
      meta graphql.query execute -q 'query($n:Int!,$l:Int!){ sev_event_from_number(number:[$n]){ chat_messages(limit:$l, load_as_viewer:false){ is_list_truncated messages { author { unixname } timestamp body } } } }' -V '{"n":674219,"l":20}' -o json   # ← substitute this tick's SEV number for 674219
      ```
      Parse `data.sev_event_from_number[0].chat_messages.messages[]` (same hypotheses/paste/ETA extraction as the CLI path). Cite `[VERIFIED via GraphQL sev_chat — CLI ACL-blocked]`. Only emit `[UNAVAILABLE]` if BOTH the CLI and this GraphQL read fail. Verified 2026-06-13 returning live messages on S674219/S673655/S664106. **Scope: SEV war-rooms only** — there is no equivalent GraphQL body-edge for general spaces (so `ot-ingest-gchat`'s watch-list reads have no such fallback). Detail: `cheatsheets/comms/gchat.md` § Read-fallback ranking; memory `reference_gchat-read-paths-graphql-fallback`.
      ...
      ii-a. **R20 mandatory pre-step — SAME-WORKLOAD RECURRENCE check (4-source).** History repeats on the same workload. BEFORE finalizing standing hypothesis, check the SAME model's prior incident history across 4 sources (extended 2026-05-17 thread `Uc-pVBEXNQ8` 11:18 PT per P-003 generalize-to-system-rule):
         ...
         **S4 — Mega-learnings cluster evidence (first-class signal):**
         4. `grep -lr "<MODEL_ID>" ~/notes/users/dennyzhang/projects/mrs-ot-agent-context/learnings/digests/`. Hits in `failure-patterns.md` = bot already canonical-mapped this model to CL-NNN. Surface even if no per-incident archive exists.
      ...
      ii-b. **R21 mandatory pre-step — CROSS-WORKLOAD PATTERN check.** History also repeats across related workloads. After R20:
         ...
         3. Symptom-cluster sweep: map current symptom to CL-NNN in `mrs-ot-agent-context/learnings/patterns/failure-patterns.md`. If matched, query cluster Evidence list for sibling instances.
   ...
   c. Send threaded reply with diagnosis. **LOCKED FORMAT (2026-05-16) — fixed sections, fixed order, fixed labels. Pre-publish lint enforces structure. Mirrors `ot-alert-monitor.md` so downstream parsers can consume both.**
      ...
      **CRITICAL anti-pattern (2026-05-16):**
      ...
      - **`pg` (Product Group): COMBINED attribution from `sev_type` (via `meta sevmanager.sev describe`) AND title-regex matching.** sev_type alone gives org-tree categories (Instagram = IG+Threads+Reels combined; Production = mvai-infra-mostly); title regex separates the PGs the operator cares about. SEV UI labels sev_type as "Stack".
        ...
        **Decision order (first match wins):**
        ...
        Source: 2026-05-16 thread `ZP2y-6Bdpwk` operator-driven design — verified that `sev_type=Instagram` covers IG+Threads+Reels (org tree), and most FB SEVs come in as `sev_type=Production` with cfr_main_feed/ifr_main/fbr titles. Title-regex layer separates them into operator-meaningful PG buckets. PG reference table with full data at `mrs-ot-agent-context/learnings/patterns/failure-patterns.md` § "PG (Product Group) reference".
      ...
      **JSON moves to paste (added 2026-05-17 thread `SN72CzuckRQ` after operator: "do I really need to see it as a human?").** The JSON block IS machine-readable metadata for validator + curation + cluster-mapping consumers — they read it. Operator does NOT. Render it OFF the gchat surface:
      ...
      JSON_PAYLOAD=$(cat <<EOF
      {
        ...
        "validator_panel": {"scope": "pending", "depth": "pending", "data_grounding": "pending", "codex": "pending"}
      }
      EOF
      )
   ...
   d. **VALIDATOR PANEL — multi-lens adversarial verification (upgraded 2026-06-07; replaces the single-Claude validator).** Goal: maximize triage correctness for a no-human-in-the-loop autonomous post. The old single validator shared the diagnosing model's blind spots — its "independence" was partly theater. This panel runs independent lenses, each with a DISTINCT failure-mode focus, votes, and writes the aggregate + per-lens breakdown to the paste JSON.

      **Mechanism — Agent tool, PARALLEL, NOT the Workflow tool.** Spawn the lenses as concurrent `Agent` subagents in ONE message (they are independent → run in parallel). Each lens is a FRESH context that sees ONLY the published diagnosis + paste JSON, NEVER the triage reasoning. **Do NOT use the `Workflow` tool here:** it returns asynchronously and its background-completion emits an intermediate `result` event reading "The workflow is running in the background…"; the daemon delivers a cron's final response verbatim, so that narration could leak to the team space. Agent fan-out is synchronous, single-result, leak-safe, and already proven in this cron path. (Smoke-tested 2026-06-07; see memory `workflow-tool-runs-from-cron-path` for the daemon result-capture gate that keeps the Workflow tool out of the LIVE cron until instrumented.)

      **Lenses** — each: ≤200-word focused prompt, 2-3 min budget, returns EXACTLY ONE of `CONFIRM` / `DISCREPANCY: <one-line + the contradicting data>` / `UNAVAILABLE: <reason>`:

      1. **Scope/boundary lens (HIGHEST value — the S657101 falsifier).** Re-run `scope_check` fresh on EVERY SEV in the cluster (step 4.5 invocation) AND re-classify the diagnosed root-cause pipeline stage (R18). `DISCREPANCY` if any SEV is actually out-of-MRS-org OR the diagnosed stage is T4/out-of-pipeline (i.e. we should not have posted/tagged). This lens's verdict is the AUTHORITATIVE auto-tag gate (step 9.e).
      2. **Root-cause-depth lens (anti-shallow-pattern-match).** Adversarially try to FALSIFY the standing hypothesis and verify the diagnosis didn't stop at a CONSEQUENCE. Re-run whichever cause-vs-consequence probes apply: trainer-liveness mvai_metrics probe (i-0c), recurring-flow-enabled check (i-0b/R15), STUS-vs-trainer entrypoint (i-0/R14), upstream-infra SEV sweep (i-0a). `DISCREPANCY` if data supports a deeper/different root cause than the one published.
      3. **Data-grounding lens (anti-hallucination).** For EACH `[VERIFIED via <cmd>]` claim in the published diagnosis, re-run the cited command fresh and confirm the claimed fact actually appears in the output. `DISCREPANCY` if any cited fact is unsupported, stale, or fabricated. (Enforces "Self-Reporting From Data" on the autonomous path.)
      4. **Codex cross-model lens (CONDITIONAL — only when a code pointer exists).** Run ONLY if the diagnosis carries a `path:line` / `file:line` code pointer in *Hypothesis & implication* or *Next actions* (2026-06-02 codex-pilot scope: code/diff-bearing triage only). Invoke headless: `cd <repo-or-fbsource>; codex exec --skip-git-repo-check "<adversarial: refute this root-cause/fix at the cited file:line; terse; ≤3 findings as 'file:line - issue - why'; default to FLAGGING if uncertain>" < /dev/null`. Map codex output to `CONFIRM`/`DISCREPANCY`. **If codex errors / is unavailable in the cron path (auth/cert UNVERIFIED) → `UNAVAILABLE`; NEVER block, NEVER skip the other 3 lenses.** Pure metadata / no-code-pointer diagnoses simply do not run this lens (`not_run` ≠ `unavailable`).

      **Aggregation (vote) → `validator_status`:**
      - `confirmed` — NO lens returned `DISCREPANCY`, AND the scope lens plus ≥1 other lens returned `CONFIRM` (not everything fell back to UNAVAILABLE).
      - `discrepancy: <merged one-liners + which lens(es)>` — ANY lens returned `DISCREPANCY`. Conservative by design: one real contradiction flags the triage.
      - `unavailable` — ALL run lenses returned `UNAVAILABLE` (e.g. Agent tool unavailable in cron context — see L6 in `~/.myclaw-ot-bot/spaces/AAQAVOjYc80/learnings.md`). Same terminal state as the pre-2026-06-07 single-validator fallback; NEVER fabricate a `confirmed`.

      **In-place update of PASTE (NOT new gchat message, NOT edit gchat message text).** After the panel returns, EDIT the paste content (created in step 9.c) via `pastry <paste_id>` overwrite: set `"validator_status"` to the aggregate above, and add a `"validator_panel"` object for the auditor: `{"scope":"<verdict>","depth":"<verdict>","data_grounding":"<verdict>","codex":"CONFIRM|DISCREPANCY|UNAVAILABLE|not_run"}`. Gchat message stays unchanged (paste link is stable). **No prose `*Validator*:` line is emitted** (retired 2026-05-17) — status lives in the paste JSON only. Sources: 2026-05-16 `6pKeH_XqjcE` (no two-message-with-gap) + 2026-05-17 `SN72CzuckRQ` (JSON off the gchat surface).

      **Measurement substrate (per-SEV `triage_events` write, prompt-header §9):** `--validator-outcome` = the aggregate enum (`confirmed` | `discrepancy` | `unavailable`).

      **Leak / reliability invariants (HARD):** the panel performs ZERO gchat sends or edits (paste-only). Its lens outputs NEVER become the cron's final response (final stays `HEARTBEAT_OK` or the step-10 post-block per L75/L81). A lens that throws counts as `UNAVAILABLE` and never aborts the run. Codex unavailability degrades to Claude-only — never to no-validation.
   ...
   e. AUTO-TAG — decoupled from hypothesis confidence (2026-05-01 fix). Fires on **signal-class evidence**, NOT mechanistic-hypothesis confidence. Tag if ALL hold:
      ...
      - validator PANEL (step 9.d) did NOT return a scope-lens `DISCREPANCY` (panel `confirmed` OR `unavailable`/degraded are both fine; an all-`UNAVAILABLE` panel is NOT a skip reason — only an explicit scope-lens `DISCREPANCY` is). Depth- or data-grounding-lens discrepancies do NOT block the tag (the tag is signal-class-based, not hypothesis-based) but ARE recorded in `validator_status` for the auditor.

      Run: `meta sevmanager.sev update --sev=S<id> --add-tag=mvai-online-training`
      Log LITERALLY: "Auto-tagged ✓" or "Auto-tag failed: <stderr verbatim>". Skip ONLY if the validator panel's scope lens flagged a `DISCREPANCY`, OR scope_check returned in_scope=false at any step. **Mechanistic hypothesis uncertain is NOT a skip reason; a depth/data-grounding-lens discrepancy is NOT a skip reason; an all-`UNAVAILABLE` panel is NOT a skip reason.**
   ...
   f. Add every SEV sev_number in cluster to `diagnosed_ids` using v3 schema: `diagnosed_ids[<sev_number>] = {"added_epoch": <now_epoch>, "notification_outcome": "$NOTIFICATION_OUTCOME"}` where `$NOTIFICATION_OUTCOME` was captured in step 9.a's send-discipline block. **HARD GATE (T273158617 Fix 7):** if `$NOTIFICATION_OUTCOME` is unset OR starts with `ERROR:`, keep the entry with the `ERROR:` outcome — DO NOT silently mark with a fabricated `POSTED:` value. `ot-cron-health-guard` step 6.7 will pick up the ERROR for triage. For backfill/preemptive/procurement silent-add paths (step 3 backfill, step 2.A' mitigated, [Preemptive], procurement-exclusion): set outcome to `OOS:<reason>` (e.g. `OOS:preemptive_launch`, `OOS:procurement_hardware`, `OOS:mitigated_backfill`).
...
10. After loop: persist state, update last_run_epoch, **persist `gchat_health` block from step 0.5 counters**, respond with HEARTBEAT_OK + per-SEV summary line that **MUST include the bot's posted gchat thread URL AND the SEV URL** (for ot-human-attention-brief link extraction):
    ...
    **DELIVERY DISCIPLINE (HARD, 2026-05-30 — operator: "you have sent many msgs… only keep the useful ones"):** the daemon posts your final response to GChat verbatim after stripping a leading `HEARTBEAT_OK`. Therefore: (1) NEVER emit narration/preamble as final text — no "State updated, lock released.", no "Composing final output.", no "Now posting the alert.", no "Records written." The per-SEV triage post is sent via the explicit `meta google.chat.message send` in step 9; that IS the message. (2) On a no-op run (0 new SEVs) respond EXACTLY `HEARTBEAT_OK` (+ optional `{metrics}` on the same line) — do NOT emit the run-summary block below (a "SEVs processed: 0" block leaks to chat as spam). (3) The run-summary block below is emitted ONLY when ≥1 SEV was actually posted this run, and ONLY as the `HEARTBEAT_OK`-prefixed trailer (for ot-human-attention-brief extraction) — never preceded by prose.
...
19. [2026-05-29 L69] **Step-3 regex must catch `OT` abbreviation in SEV titles.** SEV S669019 "Reels LSR MB9 - OOMs on OT jobs" was untagged AND used "OT" abbreviation (not "online training" spelled out) → fell through both Query A and step-3, flagged as a miss in the 10:54 PDT run. Added `\bOT[\s._-]?jobs?` to step-3 regex to catch this class. Source: ot-sev-monitor 2026-05-28T10:54 self-reported missed detection.

20. [2026-05-30 L72] **Step-3 regex must also catch "OT training" abbreviation form.** S669681 "OT training runs producing NaN immediately on startup" (self-noted by cron in 2 consecutive runs: 11:53 and 14:55 PT 2026-05-29) used "OT training" — not covered by `\bOT[\s._-]?jobs?` (catches "OT jobs") nor `online[\s._-]?train` (requires "online" prefix). S669681 was sev_type=Ads (OOS confirmed by scope_check), so no practical miss this instance; but MRS-owned SEVs with "OT training" in title would silently miss Query B. Added `\bOT[\s._-]?train` as separate OR clause in step-3 regex. Source: ot-sev-monitor 2026-05-29T11:53 + 2026-05-29T14:55 self-reported regex gap.

21. [2026-05-31 L73] **`diagnosed_ids` dedup regression: S669904 triple-notified (01:06, 02:00, 05:04 PDT on 2026-05-30) despite L45 Rule 11.** L45 added int/str normalization + second-pass file-read check (2026-05-25), but 3 consecutive runs treated S669904 as new with differing verdicts (REAL_OT_FAILURE → CONVEYOR_REGRESSION → REAL_OT_FAILURE) and different owners paged (yucheng, lupaul). Root cause TBD — likely state file not re-read fresh between consecutive runs within the 3600s lock window, OR lock state left stale from run 01:06 preventing run 02:00 from reading the updated `diagnosed_ids`. Fix: at step 9.f (state write), AFTER writing `diagnosed_ids`, immediately re-read the file and assert the key is present (no-op on success). If assertion fails → set `notification_outcome="ERROR:state_write_verify_fail"`, log `state_write_verify_fail` in run summary, treat as PENDING_RETRY (same as Rule 10/L44) so next run re-triages with fresh state. Source: 2026-05-30 S669904 three consecutive triage notifications sent to yucheng + lupaul.

22. [2026-05-31 L74] **MONITOR/confidence:low verdict → operator 1:1 only; NEVER team space.** Channel routing rule: (a) PAGE or REAL_OT_FAILURE (any confidence) → team space `spaces/AAQA2bZMw24` AND operator 1:1; (b) MONITOR → operator 1:1 `spaces/AAQAVOjYc80` ONLY (regardless of confidence); (c) NO_ACTION / OOS / TRANSIENT_NOISE → suppress, state-write only. S669985 (2026-05-31T01:00) was routed to team space despite MONITOR/confidence:low verdict — violates the TEAM-SPACE GATE at top of this prompt. The gate already says "bot cannot fully handle → team space; bot fully handled at HIGH confidence → operator 1:1 only." MONITOR/low is ambiguous-not-PAGE; it belongs in operator 1:1 where Denny can review without alerting the whole team. Source: ot-sev-monitor 2026-05-31T01:00 S669985 bot reply `chat.google.com/room/AAQA2bZMw24/Z7V6kNvAM9M`.

23. [2026-05-31 L75] **Final response MUST be exactly `HEARTBEAT_OK` when any explicit gchat send was executed this run.** If run summary prose appears as the final response (delivered=delivered with non-HEARTBEAT_OK content), it causes double-delivery: once via explicit `meta google.chat.message send`, once via daemon auto-deliver of the final response. Symptom: ot-sev-monitor 2026-05-30T02:00 raw_response started with "State updated, lock released. Composing final output." + triage block; delivered=delivered → content posted twice. Guard: before terminating, if `SENT_COUNT > 0` (any explicit gchat send succeeded this run), verify the final response is EXACTLY `HEARTBEAT_OK`. Pattern: narration emitted BEFORE the final HEARTBEAT_OK line causes daemon to auto-deliver the entire response including narration. Enforce: generate HEARTBEAT_OK as the first and only output line when sends occurred. Source: 2026-05-30T02:00 ot-sev-monitor double-delivery of S669904 triage via narration-leak bypass.

24. [2026-06-05 L81] **State-management narration before HEARTBEAT_OK causes delivery (EXTENDS L75 beyond explicit-send guard).** L75 guards against double-delivery when `SENT_COUNT > 0`. This extends to ALL runs: any text appearing before HEARTBEAT_OK in the final response gets delivered by the daemon verbatim — including the literal string "HEARTBEAT_OK" — even when no explicit gchat send occurred. "State written and verified. Run complete." before HEARTBEAT_OK + run summary causes the narration AND "HEARTBEAT_OK" to appear as literal text in the GChat message. Final response MUST be (a) HEARTBEAT_OK alone (daemon skips), or (b) the formatted post-block starting with `---` with NO embedded HEARTBEAT_OK or preceding narration. State writes, verification confirmations, and lock-release messages are internal — never appear in final response. Source: ot-sev-monitor 2026-06-04T09:56 — "State written and verified. Run complete." delivered to GChat as prefix before the run summary; 3 confirmed violations across ot-sev-monitor + ot-alert-monitor 2026-06-04/05.

25. [2026-06-10 L1] **"cannot restart" regex coverage for LSR teacher model failures.** S673655 "Threads LSR teacher model (2128461099) cannot restart" was OT-relevant but missed by all Query A and Query B regex patterns — not tagged, not triaged. Root: "restart" as a failure verb is absent from step-3 OT signal-class regex. Add `\bcannot\s+restart\b` (case-insensitive, word-boundary) as a step-3 OR clause. Apply in both the Query B title-match regex AND `sev_identification._EXPLICIT_PATTERNS_DEFAULT`. Source: S673655 miss noted in S673808 run 2026-06-09T15:35.

26. [2026-06-10 L2] **diagnosed_ids S-prefix normalization (extends L45 int/str fix).** ot-sev-monitor 2026-06-09T22:10 — state_expansion_anomaly=true: 47 SEVs appeared new (all already tracked; net new: 1) due to S-prefix/numeric key mismatch in step-5. L45 (rule 11) normalized int→str but did not normalize S-prefixed strings vs bare numeric strings ("S673808" ≠ "673808"). Fix: normalize all SEV ID keys to S-prefixed format at BOTH write and read time: `sev_key = f"S{str(sev_id).lstrip('S')}"` so both "S673808" and "673808" map to "S673808". Apply in every diagnosed_ids write and membership check. Source: ot-sev-monitor 2026-06-09T22:10 run summary state_expansion_anomaly=true.

27. [2026-06-12 L1] **OOS-SEV state-bloat dedup miss: Query A tagged-but-OOS SEVs must NOT fill diagnosed_ids.** ot-sev-monitor 2026-06-11T13:55 — S674670 (CONVEYOR_REGRESSION, already diagnosed 3.8h prior at 09:48) was re-diagnosed and duplicate PAGE sent due to state expanding from 166→252 IDs (delta=86 tagged-but-OOS SEVs added by Query A — all scope_check_false or wrong-org). S-prefix normalization (rule 26) was already applied; the miss was a different mechanism: bulk OOS additions inflated diagnosed_ids beyond the valid tracked set, disrupting membership lookup for S674670. Fix: track OOS SEVs (scope_check_false or wrong-org) from Query A in a SEPARATE `oos_seen_ids` set, NOT in `diagnosed_ids`. Membership check for dedup must only consult `diagnosed_ids` (in-scope triaged SEVs). Do NOT merge the two sets. Additionally: when state_expansion_anomaly=true (delta>40 in one run per rule 9), run a post-expansion dedup verification pass — re-check all in-progress SEVs still active this run against the expanded diagnosed_ids before committing any new PAGE verdicts. Source: ot-sev-monitor 2026-06-11T13:55 S674670 duplicate notification; state_expansion_anomaly=true delta=86.

28. [2026-06-12 L2] **"OT failing" regex coverage (extends rules 19, 20, 25).** S674760 "[Feed ESR] SMSL ST model m2124455849 (old prod) OT failing consistently" was missed by all step-3 Query B patterns (`\bOT[\s._-]?jobs?`, `\bOT[\s._-]?train`, `\bcannot\s+restart\b`) — caught only via manual scope_check probe. "OT failing" is a distinct failure-description phrase. Add `\bOT\s+fail` (case-insensitive, word-boundary on OT) as a step-3 OR clause. Apply in Query B title-match regex AND `sev_identification._EXPLICIT_PATTERNS_DEFAULT`. Note: avoid bare `\bOT\b` (too broad — "OT" appears in other contexts); `\bOT\s+fail` is scoped to failure descriptions. Source: ot-sev-monitor 2026-06-11T11:54 self-reported regex miss on S674760.

29. [2026-06-12 L5] **mtml keyword alone is insufficient for MRS-org admission when primary oncall is IG-product-only.** S675298 (oncall=instagram_explore_core_modeling) was admitted for triage via `mtml` keyword match in step-3. Validator discrepancy: "oncall=instagram_explore_core_modeling IG org, mtml keyword match insufficient for MRS scope." Bot correctly skipped auto-tag but still spent triage resources. Rule: after step-3 matches on `mtml` as the ONLY or PRIMARY OT-signal keyword (no other MRS-specific signal in title such as mvai, MAST, TGIF, online train, LSR, hedwig, silvertorch), check the primary oncall from SEV metadata. If oncall contains `instagram_` prefix or matches known IG-product oncalls (instagram_explore_*, instagram_feed_*, ig_*) WITHOUT an MRS-org co-occurrence signal — apply the IG-default-admit keyword gate (same gate as sev_type=Instagram check in team_lane_scope). If gate fails → in_scope=false, add to diagnosed_ids without full triage. This prevents wasting triage budget on IG-owned SEVs that mention mtml in titles. Source: ot-sev-monitor 2026-06-12T08:04 S675298; validator scope=DISCREPANCY.

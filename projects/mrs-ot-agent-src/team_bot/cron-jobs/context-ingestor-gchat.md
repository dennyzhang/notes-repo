[context-ingestor-gchat cron] Weekly Mondays 09:00 PT. Pull the last 7 days of activity from a watch-list of external OT-adjacent gchat spaces, distill **new project context** the OT Master Agent should integrate, and write one catch-up file per space to `~/notes/users/dennyzhang/projects/mrs-ot-agent-context/auto-learnings/digests/<YYYY-MM-DD>-<space-slug>-catchup.md`.
...
State file: `~/notes/users/dennyzhang/projects/mrs-ot-agent-src/state/context-ingestor-gchat-state.json` — `{"last_run_epoch": <int>, "per_space": {"<space_id>": {"last_catchup_date": "<YYYY-MM-DD>", "last_msg_create_time": "<iso>", "skip_until_epoch": <int>}}}`.
...
Output: `~/notes/users/dennyzhang/projects/mrs-ot-agent-context/auto-learnings/digests/<YYYY-MM-DD>-<space-slug>-catchup.md` — one file per space per run. Reference output: `2026-05-18-rtinfra-ws2-catchup.md` (first instance, written manually by operator).
...
2. **For each space in the watch list:**
   ...
   f. **Build catch-up file** at `<context_root>/<YYYY-MM-DD>-<space-slug>-catchup.md` with:
      ...
      _Auto-distilled by `context-ingestor-gchat` cron. Source: <N> human messages spanning <first_msg> → <last_msg> in **<display_name>** (<member_count> members; primary contributors: <top-3 senders by msg count>)._
...
4. **Post run summary to operator gchat (spaces/AAQAVOjYc80):**
   ...
   _State: per_space updated · next run <next Monday 09:00 PT>_
   ```
...
- Does NOT alert on individual messages — only weekly distillation

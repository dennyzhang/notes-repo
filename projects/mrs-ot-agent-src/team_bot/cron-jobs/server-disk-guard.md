...
2d. **Preventive worktree-clone reap (EVERY run, ALL states — added 2026-06-02).** The Claude Code `isolation: worktree` leak is *continuous*, but step-4b mitigation only fires on a state *transition* — so a disk stuck at `critical→critical` would never self-heal. Reap on every tick, independent of state, so leaked clones never accumulate to a fill. SAFE + idempotent: only bot-user-owned `/tmp/.tmp*` dirs older than 60 min holding NO bind mounts (active agents are recent; genuine tupperware sandboxes have submounts and are skipped — verified 2026-06-02: 0 of 2056 leaked dirs had any active mount).
   ```bash
   wt_reaped=0
   for d in $(find /tmp -maxdepth 1 -name '.tmp*' -type d -user "$(whoami)" -mmin +60 2>/dev/null); do
     grep -q " ${d}" /proc/mounts && continue   # never rm-traverse an active mount
     rm -rf "$d" 2>/dev/null && wt_reaped=$((wt_reaped+1))
   done
   ```
   Persist `state.last_worktree_reap = {epoch, count}`. Count root-owned `/tmp/.tmp*` (NOT user-reapable — need sudo); only when that count is >50 or rising, surface it ONCE in the next warning/critical alert for operator/sudo follow-up. DURABLE fix is upstream: Claude Code must clean its worktree temp on agent exit — this sweep is the safety net, not the cure.
...
4. **Compare to prior state** in state file:
   ...
   - `warning → warning` or `critical → critical`: bump `consecutive_in_state`. No post (suppressed; post recovery only). NOTE: the preventive worktree reap (step 2d) still runs every tick — that is how a stuck-`critical` disk self-heals without a transition, even though 4b mitigation is transition-gated.
   ...
   - `warning → critical` or `critical → warning`: post severity change. Reset. **`warning → critical` also triggers auto-mitigation (step 4b), including the Tier-2 aggressive pass.**

4b. **Auto-mitigation (fires on any transition INTO warning or critical: ok→warning, ok→critical, warning→critical).** Capture reclaimed bytes per step; append the reclaim summary to the alert body (accounting block below).

   **Tier 1 — SAFE pass (always, on entering warning OR critical).** Run in order. The worktree-clone sweep below is NOT rate-limited (it only removes >60min orphans — safe + idempotent, and it is the dominant reclaimer); the eden/dotslash/backup steps are rate-limited to 1 per 4h (`state.last_auto_mitigation_epoch` gate) to prevent eden-gc thrash on flapping.
   - **Orphaned worktree/sandbox clones** are already reaped on EVERY run (step 2d) — the biggest reclaimer (2026-06-02 root cause: Claude Code `isolation: worktree` leak filled 2.3 TB via ~2,355 un-cleaned `/tmp/.tmp*` clones, ~54 GB each). No separate pass here; the alert reports `wt_reaped` from step 2d plus any rising root-owned-leak count.
   ...
   **Tier 2 — AGGRESSIVE warm dotslash reclaim (ONLY when state is `critical` AND the mount is STILL critical after Tier 1).** Fast-fill escalation. The dotslash launcher cache is content-addressed and re-fetches on demand, so trimming *warm* entries is SAFE — re-download latency only, no data loss. The Tier-1 >30d cutoff reclaims ~nothing in a fast fill: **2026-06-02 — dotslash was 46 GB across 692,700 files, ALL <30d, so the safe pass freed 0 GB and the mount stayed CRITICAL**; Tier 2 closes that exact gap. NOT subject to the 4h rate-limit (a critical fire can't wait 4h), but gated to 1 per 20 min via `state.last_aggressive_mitigation_epoch` to avoid thrash. Step the atime cutoff DOWN, re-checking free space after each step, and STOP as soon as the mount clears critical (reclaim only what's needed, keep warm cache otherwise):
     ```bash
     for days in 7 3 1; do
       find ~/.cache/dotslash -type f -atime +$days -delete 2>/dev/null
       free_gb=$(df -BG / | awk 'NR==2{gsub(/G/,"",$4); print $4}')
       [ "$free_gb" -ge 30 ] && break   # critical free-threshold (20 GB) + 10 GB headroom
     done
     find ~/.cache/dotslash -type d -empty -delete 2>/dev/null
     ```
   - If still critical after the `days=1` pass → tag `AUTO-MITIGATION-INSUFFICIENT` (step 8). Do NOT auto-escalate to the operator-only list.

   **Accounting:** re-run `df -BG /` and append to the alert body: `Auto-mitigation reclaimed: <wt> GB (worktree clones) + <eden> GB (eden) + <ds30> GB (dotslash>30d) + <dswarm> GB (dotslash warm/Tier-2) + <bk> GB (backups) = <total> GB. Post-mitigation: <pct>% used, <free_gb> GB free.` (worktree-clone reclaim is usually the dominant term.) Set `state.last_auto_mitigation_epoch = now()`; if Tier 2 ran, also `state.last_aggressive_mitigation_epoch = now()`.

   - **NEVER auto-run** the following — they are operator decisions: `btrfs balance` (I/O heavy, wasted unless Device-unallocated low), `sl strip` (can lose draft work), blanket `rm -rf /tmp/.tmp*` (would hit root-owned bind mounts + other users' dirs — only the bot-user-owned, mount-free, >60min subset is safe, and that is handled in the Tier-1 worktree sweep above), VSCode install pruning (active dev environments).
...
5. **Identify top 3 disk consumers** when posting warning/critical:
   ...
   **`/tmp/.tmp*` — TWO distinct kinds, do not conflate (corrected 2026-06-02):**
   1. **Bot-user-owned worktree/sandbox home-clones — REAL, ~46 GB each, the #1 fill source.** Leaked by Claude Code agent isolation (see Tier-1 sweep). These ARE real disk consumers and ARE cleanable (no bind mounts inside). On 2026-06-02, 2,355 of these filled the 2.3 TB disk. Discriminate by `-user "$(whoami)"` + absence from `/proc/mounts`.
   2. **Root-owned tupperware sandbox / container-overlay mount points** — each holds bind-mounts of `dev`, `usr/local/fbcode/...`, `usr/facebook/tupperware/hostagent/...`. `du --max-depth=1 /tmp` *without* `-x` crosses these and reports inflated/fictional sizes (saw 832 GiB apparent vs 573 GiB real). NOT user-cleanable; never `rm` them. Always pass `-x` to `du` when scanning `/tmp`.

   **Earlier error this corrects:** the prior note claimed ALL `/tmp/.tmp*` were fictional root-owned bind mounts. That blanket framing caused a 2026-06-02 misdiagnosis — dismissing a real 2 TB worktree-clone leak as "not real." Always split by owner + `/proc/mounts` first. In the alert, report kind-1 (count, total real size, reclaimable) separately from kind-2 (count, apparent only).

6. **Suggested mitigations** to include in critical alert (after the auto-mitigation summary):
   - `du -sh ~/.cache/*` — `~/.cache/dotslash` can hold tens of GB (46 GB on 2026-06-02). On `critical` this is now auto-reclaimed by Tier-2 (step 4b); only surface a manual `rm -rf ~/.cache/dotslash/*` (safe, re-fetches on demand) if Tier-2 was rate-limited or insufficient.

[ot-cron-health-guard cron] Hourly. Audit the OT bot's own cron job runs to surface silent failures. Seven failure classes detected:
...
2. **Pull manifest.** Read `~/notes/users/dennyzhang/projects/mrs-ot-agent-src/team_bot/cron-jobs/MANIFEST.json` to get the canonical list of `(job_id, schedule_type, cron|interval_seconds)`. EXCLUDE this cron (`ot-cron-health-guard`) from audit to avoid recursion.
...
6.6. **Missed-completion audit (class 5 — Evergreen kill).** Detect the pattern: daemon log shows `Scheduled job firing: <job_id>` line at timestamp T, but (a) NO matching completion log line within `expected_window`, AND (b) NO `job_runs` row with `run_at >= T` AND `run_at <= T + expected_window`. Means the cron's pi-harness subprocess was killed mid-flight — most commonly by an Evergreen restart whose drain only covers thrift jobs (not pi sessions), but also catches OOM kills, manual SIGTERM, or systemd unit failures.
   ...
   **Cross-check for Evergreen kill signature:** if multiple candidates share the same `T_epoch` window (±2min), AND the daemon log shows `Evergreen: session drained, re-execing process` OR `MyClaw <build> starting` within 5 min of `T_epoch`, classify all candidates as `evergreen_kill` (one-line root cause for the alert). Otherwise classify as `missed_completion` (other root cause: OOM, SIGTERM, systemd, network).
   ...
   - **Auto-mitigation (safe)**: nudge `UPDATE jobs SET next_run_epoch=NOW WHERE id='<job_id>'` to fire a fresh run if EITHER (a) the job is on the interval-only kill-allowlist (step 7.5 v3 hung allowlist), OR (b) the job is on the **retrigger-safe cron-type allowlist** (step 7.5, defined below). **Critical distinction: this mitigation is a RETRIGGER, not a kill** — for `evergreen_kill` the pi-harness process is already dead, so there is nothing to SIGKILL and the write-heavy kill-prohibited list (which guards mid-write SIGKILL on the `*→hung` path) does NOT apply. Bounded, idempotent. (2026-06-08: `ot-triage-summary` fired Jun 8 09:30, killed in-flight by the 09:34 Evergreen restart, never re-queued — fell in the detect-only gap because cron-type jobs were excluded from ALL mitigation. Root-fixed by separating retrigger from kill.)
...
7.5. **Auto-mitigation pass** — for transitions where a SAFE, SCOPED mitigation exists, attempt it BEFORE escalating. If mitigation succeeds, downgrade the transition to `auto_mitigated` and post a brief mitigation report instead of the original alert.
   ...
   | `*→missing` / `*→evergreen_kill` on a **retrigger-safe cron-type (daily/weekly) job** (v4, 2026-06-08 thread `Iy87Mr4jFIM`) | Direct sqlite UPDATE `next_run_epoch=now` — **NO process kill** (for `missing` nothing ever fired; for `evergreen_kill` the process is already dead) | The kill-prohibited list (hung-gate #2) guards SIGKILL-mid-write on a RUNNING write-heavy cron; a pure retrigger of a NOT-running job has zero partial-write risk, so that list does not apply here. Gated by the retrigger-safe cron allowlist below (idempotent-by-design only). |
   ...
   **v4 — `*→missing` / `*→evergreen_kill` retrigger for cron-type (daily/weekly) jobs (added 2026-06-08 thread `Iy87Mr4jFIM`):**

   This is the SAME sqlite `next_run_epoch=now` retrigger as the interval `*→missing` case above — it does NOT kill any process — so it is safe for write-heavy cron-type jobs that the `*→hung` kill path correctly refuses. Use it ONLY for `*→missing` (job never fired in its window) or `*→evergreen_kill` (job fired but its already-dead session left no completion + no DB row). NEVER use it for `*→hung` (a still-running process — that needs the v3 SIGTERM path and its interval-only gates).

   **Retrigger-safe cron-type allowlist** (idempotent-by-design — every re-run converges to the same state via a state-file/dedup or `--draft`; no destructive or order-dependent window): `ot-triage-summary` (resolved-sev summaries, idempotent via `triage-summary-state.json`), `ot-knowledge-distillation` (always `--draft`, ≥3-incident dedup, cumulative corpus). **Start small; expand only after confirming a candidate cron is re-run-idempotent.** A cron-type job NOT on this list still gets detect+alert (no auto-retrigger) until proven idempotent.

   **Safety gates (ALL must pass) — no-kill variant, lighter than the hung gates:**
   1. Transition is `*→missing` OR `*→evergreen_kill` (NOT `*→hung`).
   2. Job is on the retrigger-safe cron-type allowlist above.
   3. No clean `job_runs` row for this job since its missed/killed slot (don't double-fire a job that already self-recovered).
   4. The job has NOT been auto-retriggered in the last 6h (dedup via `last_auto_mitigation_epoch`; if it needs retriggering twice in 6h, there's a deeper bug — escalate).
   5. Respect the 3-mitigations-per-run cap (shared with the interval path).

   Then run the same verify loop (poll `job_runs` ≤5 min for a new `status='ok'` row → `auto_mitigated`; else keep the original transition + escalate). For dependency-ordered pairs (e.g. `ot-triage-summary` feeds `ot-knowledge-distillation` 4h later), retrigger the upstream first and let the downstream's own scheduled run consume the refreshed corpus — do not force-chain.

   **v3 — `*→hung` auto-mitigation (added 2026-05-17 thread `4GTCGhniXXU`):**

   **Safety gates (ALL must pass):**
   1. Job is on the **interval-only allowlist**: `ot-sev-monitor, ot-alert-monitor, ot-post-monitor, ot-prompt-change-validator, ot-cron-health-guard (excl-self), ot-notes-deletion-watch, ot-notes-commit-push, server-disk-guard, ot-thread-summarizer, ot-fbpkg-cap-watch`. These are all READ-ONLY crons whose only external write is gchat post. Killing them mid-run loses at most the current run's findings; next schedule re-discovers.
...
7.7. **ROOT-FIX HANDOFF — turn a persistent, agent-code failure into a diff-loop task (2026-06-07, operator: "do you have an autonomous workflow to debug and root fix the problems? north star >95% pass rate").** Detection+escalation alone never fixes a real bug — close the loop into the diff-loop (`ot-knowledge-distillation` step 3.5). For a `*→persistent_failure` transition (≥3 consecutive, NOT transient) **whose root cause is the agent's OWN cron prompt/code** — i.e. the step-7 triage points to `stale path reference`, `schema mismatch`, `prompt bug`, `gchat_wrapper_bypass`, or similar fixable-in-`mrs-ot-agent-src` cause — file ONE deduped `ot-agent-self-improve` task so the diff-loop drafts the fix:
   - **EXCLUDE non-agent-code causes from the FIX task:** `upstream API change`, infra/`Claude session init failed`, `evergreen_kill`/restart artifacts, auth, external service. Those are NOT diff-loop-fixable (the bot is read-only on external code) — do NOT file a diff-loop *fix* task.
   - **UPSTREAM branch — P-017 decisive-metric task (added 2026-06-09 thread `pnL5GSkFRW0`, operationalizes P-017).** For an excluded (upstream) cause that is ALSO recurring (≥3 in the audit window) AND high-confidence, escalation-only is not enough — the symptom will keep recurring while the upstream owner works it, and re-narrating it each run is the noise P-017 forbids. Instead file ONE deduped, handhold-first **decisive-metric** task (distinct from the diff-loop fix task) by calling the shared helper:
     ```bash
     P017_RESULT="$(bash ~/notes/users/dennyzhang/projects/mrs-ot-agent-src/tools/file-decisive-metric-task.sh \
       "<stable-issue-key>" "<one-line upstream issue>" "<root_cause_T#### if any>" "<upstream_owner>" 2>/dev/null)"
     # record $P017_RESULT (TASK_CREATED:T#### | DEDUP_SKIP:T####) in state; it MUST NOT appear in your final response
     ```
     The helper dedups (skips if an open `ot-upstream-decisive-metric`-tagged owner=dennyzhang task already names the issue), so it is safe to call every run. Its deliverable is "build a decisive ground-truth metric query that confirms the issue + is the acceptance test for the upstream fix." Still escalate the failure to the 1:1 in step 8 (the task is the tracking path; the escalation is the heads-up). Record the returned `TASK_CREATED:T####` / `DEDUP_SKIP:T####` in state so it isn't refiled. (This same helper is the canonical P-017 mechanism for OTHER upstream-detectors too — ot-bot-volume-watch team-leak audit, ot-sev-monitor / ot-alert-monitor cross-team SEV recurrences, ot-fleet-health chronic upstream subsystems — wire each detector's upstream branch to call it.)
   - **Dedup:** `meta tasks.task list --tags-include-any-of=ot-agent-self-improve --status-is=Open --owner-is=dennyzhang -o json 2>/dev/null` (tolerate the not-yet-used-tag error as "none"); skip if an open task's title already names this `<job_id>`.
   - **File (handhold-first):** `meta tasks.task create --title="[ot-self-improve] fix <job_id>: <one-line root cause>" --description="Auto-filed by ot-cron-health-guard (provenance, principle #19 — title names the FAILING job, this names the FILER so the change traces to its origin). <the step-7 diagnosis + the 3 failing raw_responses>" --owner=dennyzhang --priority=MID --add-tag=ot-agent-self-improve -o json`. `--owner=dennyzhang` ONLY — no other subscriber/assignee (handhold-first); `--priority=MID` so it never lands as UNKNOWN. Record the task id in the cron-health state so it isn't refiled while open.
   - The diff-loop then drafts the fix (`--draft`, `ot_bot_autodraft` tag, 1:1); operator reviews + lands + closes. This is the autonomous debug→root-fix path; the >95% pass-rate north star is the metric it defends. Still escalate the failure to the 1:1 in step 8 too (the task is the fix path, the escalation is the heads-up).
...
10. **Always respond `HEARTBEAT_OK {jobs_audited: N, transitions: T, alerts_posted: A, recoveries: R, auto_mitigated: M, mitigation_failures: MF, ordering_bugs: O, notification_anomalies: NA, suppressed_repeats: S}`** at end of run. Suppressed-repeats counts jobs in same failing state as before (no re-alert) — the metric proves the de-spam logic is working. `auto_mitigated` counts successful retriggers; `mitigation_failures` counts attempts that fell through to alert. `ordering_bugs` counts crons with ≥1 notification-retraction pair detected this run. `notification_anomalies` counts crons with ≥1 `ERROR:*` or aged-`LEGACY_UNKNOWN` outcome detected in their state file this run (per class 6 / step 6.7).
   ...
   **ACTIVE-RUN path (transitions>0) — SAME no-preamble rule (validator persistent-FAIL since 2026-06-07, fixed 2026-06-09).** On an active run the final response is STILL EXACTLY `HEARTBEAT_OK {metrics}` with NO preceding text. The alerts were already delivered via the explicit sends in step 8 (top-line + threaded replies); the final response must NOT re-narrate them — no "State written and verified.", no "Alert posted to thread X.", no "N transitions handled." before the `HEARTBEAT_OK`. The gchat post in step 8 IS the alert; any preamble in the final response leaks a duplicate to chat. Clean AND active runs alike: the final response is exactly the `HEARTBEAT_OK {metrics}` token line and nothing else.
...
Safety:
- **Mostly read-only with TWO narrow write exceptions.** This cron reads sqlite, daemon log, and manifest. The ONLY writes are: (1) the `UPDATE jobs SET next_run_epoch` in step 7.5 for `*→missing` auto-mitigation on interval crons — a bounded, idempotent retrigger; (2) **v3 (2026-05-17):** the SIGTERM + `UPDATE next_run_epoch` for `*→hung` auto-mitigation on the allowlisted interval-only crons (see safety gates in step 7.5); (3) **v4 (2026-06-08):** the `UPDATE next_run_epoch` retrigger (NO kill) for `*→missing`/`*→evergreen_kill` on the retrigger-safe cron-type allowlist (`ot-triage-summary`, `ot-knowledge-distillation`). Never modifies job_runs, never restarts the daemon, never edits cron prompts, never SIGKILLs (only SIGTERM with escalation on failure).
- **Exclude self.** This cron must not audit `ot-cron-health-guard` to avoid recursive alarm loops if THIS cron itself fails. Operator catches its absence via missing health-check messages.

**DELIVERY = TEAM SPACE via explicit send (2026-05-30, VERIFIED WORKING).** Render the brief, then send it with an EXPLICIT `meta google.chat.message send --space-name=spaces/AAQA2bZMw24 --text="<rendered brief>"`, then respond EXACTLY `HEARTBEAT_OK` so the daemon's default 1:1 auto-delivery posts nothing (no double-post). **Proof this works:** the 2026-05-30 20:44 run did exactly this and the brief landed in `spaces/AAQA2bZMw24` at 20:43:53 (read-back confirmed). Earlier I wrongly concluded "scheduled crons can't reach the team space" — that was a misdiagnosis: the daemon log "nothing to report" / `delivered=skipped` only reflects the suppressed DEFAULT delivery, NOT the explicit send, which succeeds. **VERIFY-BY-READBACK (mandatory):** after the explicit send, the run is only "delivered" if the message is actually readable in `spaces/AAQA2bZMw24` — do not trust the send's success string alone (false-delivered class). If the explicit send errors, surface the error; do NOT silently fall back. If the brief is empty, respond `HEARTBEAT_OK` and send nothing.
...
7) **OT followups** — Actionable team-coordination items only. **Only include items that are blocked on someone and are >7 days stale.** Examples: a SEV awaiting `mvai-online-training` tag for >7d, a runbook update that's overdue, a cross-team handoff that hasn't been acknowledged. NOT operator's personal todo list. NOT a re-listing of SEVs already in section 4. If nothing meets the bar, omit the section.

**⛔ OUTPUT CONTRACT (read FIRST) — never output narration/preamble; your FINAL response is delivered to chat VERBATIM unless it is exactly `HEARTBEAT_OK`.** The real work product is the **gdoc shift-summary** (written silently). Your chat final response is EITHER (a) `HEARTBEAT_OK`, OR (b) ONE short status line (e.g. `📋 shift-summary updated: <tab> — N open comments addressed`). NEVER emit run-narration before/around it — no "State updated…", "Composing the draft…", "Records written…", "Audit complete…", no step-by-step play-by-play. Do the gdoc work silently; the one-line status or `HEARTBEAT_OK` IS the entire chat output. (Preamble-prohibition flagged absent by ot-prompt-change-validator 2026-06-23.)
...
**(0) CONCURRENCY LOCK — MANDATORY FIRST ACTION (2026-06-24 double-fire fix).** Before anything else, run `bash ~/notes/users/dennyzhang/projects/mrs-ot-agent-src/tools/shift-summary-lock.sh acquire`. If it prints `BUSY`, another run holds the lock → respond `HEARTBEAT_OK already-running` and STOP immediately (do not proceed). If `ACQUIRED`, continue — and you **MUST release the lock on EVERY exit path** (`...shift-summary-lock.sh release`): the same-day guard below, the quiet-shift early-exit, the final persist (step d), and any abort. (Why: 2026-06-24 the daemon fired at 08:40 + 08:43; both passed the same-day guard because state is written only at the END, so neither had claimed the slot when the other checked → both ran + both full-replaced the gdoc. The lock serializes runs so fire #2 is blocked.)
...
- If `last_window_end` in state == today's `WINDOW_END`: **release the lock** (`...shift-summary-lock.sh release`), then stop, respond `HEARTBEAT_OK already-ran-today`. Same-day re-trigger guard.
...
**MANDATORY PRE-PUSH SECTION-STRUCTURE LINT (HARD, 2026-06-24).** Before pushing, assert the rendered ghtml's `<h3>` section headers EXACTLY MATCH the template's, in order. This catches the format-instability regression where a run (esp. **mid-shift**) hand-composes its own structure instead of filling the template — e.g. 2026-06-24 the mid-shift run produced `§1 Headline Numbers / §2 Daily Timeline / §3 Open SEVs / §4 Alerts / §5 User Reports / §6 Hand-off Notes` instead of the canonical sections, and the operator caught the inconsistency. Mid-shift mode changes ONLY the `<h2>`/header line (`(mid-shift, …)`, "Active oncall: …") — the `<h3>` section set is IDENTICAL to the template in every mode.

```python
# Section-structure lint (run before gdocs apply/replace, after the sort lint)
import re
TEMPLATE = "~/notes/users/dennyzhang/projects/mrs-ot-agent-src/team_bot/cron-jobs/ot-shift-summary-template.html"
def h3s(ghtml): return [re.sub(r'<[^>]*>','',m).strip() for m in re.findall(r'<h3[^>]*>.*?</h3>', ghtml, re.S)]
want = h3s(open(os.path.expanduser(TEMPLATE)).read())   # ['Overview','Impact this shift','Pain Points','Hand-off (incoming oncall)','Daily Timeline']
got  = h3s(rendered_ghtml)
if got != want:
    raise SectionLintAbort(f"section structure != template.\n  want: {want}\n  got:  {got}\n  → do NOT hand-compose; fill the template sections.")
```
If it aborts, re-render by FILLING THE TEMPLATE (do not invent sections), then re-run the lint.
...
3. **SEV entry format** inside any bucket: `↳ <b><a href=fburl.com/sev/S###>S###</a></b> [L<n> <STATUS>]</span> — <1-line outcome, no prose>`. NO multi-line continuations. NO narrative paragraphs.
...
**RULE 64 — Daily Timeline MUST render EVERY day in the shift window; never omit/empty a day (2026-06-22, comments `AAAB90Bqn8Y` + `AAAB8N0Ubx4` on tab `6/23`).** Root cause: the timeline is built from `daily-brief` state files, but `daily-brief` runs **weekday 8:14 PT only** — so **weekend days (Sat/Sun) have NO state file**, and any weekday with an empty/`entries:0` state contributes nothing. Those days were being silently **omitted** (operator flagged missing 6/21 Sun) or rendered as an **empty `<h4>`** (operator flagged empty 6/17 Wed). FIX: do NOT iterate the daily-brief state files; iterate the **explicit calendar window Tue→Mon**. For EACH date emit exactly one `<h4>`; if that date has a state file, render its buckets (RULE 63); if it has NO state file or an empty one (weekends especially), **live-pull that day's signal** — `meta sevmanager.sev list ... --created-after="<day> 00:00 PT" --created-before="<day> 23:59 PT"` (VERIFIED flags 2026-06-23; NOT `--min-created/--max-created`, which do not exist), `meta oncall.feed list ... <day>`, `meta workplace.post list ... <day>` — and render the buckets, OR if genuinely nothing, the RULE-63 `<h4>YYYY-MM-DD (Day) — (quiet)</h4>` single line. **Pre-push lint:** assert exactly one `<h4>` exists for every date in the Tue→Mon window (count == 7); a gap or an `<h4>` with no following content/`(quiet)` marker → ABORT. **11pt consistency:** after rendering, run the 11pt body-font pass over the WHOLE timeline (per gdocs cheatsheet) so no single day's block renders larger than its siblings (operator flagged 6/19 Fri bigger font — caused by a day block inserted without the font-normalization pass).
...
**(d) Persist state + release lock.** Write `last_run_epoch`, `last_window_end`, `last_known_oncall = <outgoing_unixname>` to `/home/dennyzhang/.myclaw-ot-bot/spaces/AAQAVOjYc80/ot-shift-summary-state.json`. Also write `last_gdoc_append_epoch` + `last_gdoc_section_heading` to a new key for the gdoc-side dedup guard. **Then release the concurrency lock: `bash ~/notes/users/dennyzhang/projects/mrs-ot-agent-src/tools/shift-summary-lock.sh release`.** (Lock release is also required on the quiet-shift early-exit and any abort path — see step 1(0).)
...
- **Same-day rerun guard** — step 1's `last_window_end == today` check prevents accidental double-post when operator manually triggers the cron. **Backed by an atomic concurrency lock (step 1(0), `shift-summary-lock.sh`)** that serializes runs so a daemon double-fire can't slip past the guard via the check-then-act race that hit 2026-06-24 (08:40 + 08:43 both ran because state is written at END; the lock makes fire #2 either skip-on-BUSY or hit the now-written guard).

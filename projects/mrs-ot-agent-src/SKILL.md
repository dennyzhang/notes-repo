...
| Guessing ownership from body text or prior memory (R26) | Ownership changes frequently; body text describes symptoms, not the current responsible party. "mrs_online_training" as a catch-all is often wrong. | **Owner routing is a LOOKUP, not a guess.** FIRST run `bash tools/owner-lookup.sh --model-id=<id>` (or `--model-type=<type>`): it resolves owner→team→oncall deterministically (models.md Owner column → `meta people.profile get`) and prints the source. NOT_FOUND (model never had an incident) → fall back to the LIVE source: `meta ai.model describe --model-id=<model_id>` (`model_owner`), or `meta ai.mast-job metadata --name=<mast_job_name>` (`model_owner_unixname`). Fallback: "mrs_online_training". EXCEPTION: if the incident explicitly names an upstream outage (ZippyDB, Scribe, upstream SEV S<digits>) as root cause → owner = upstream service's oncall, NOT the model owner. |
...
| [human-input/models.md](human-input/models.md) | OT models derived from prod incidents (incident-reconciled + RUNNING-filtered, NOT a full roster) with entity IDs (validate edits with `tools/validate-models.py`; freshness via `tools/reconcile-models.sh`) |

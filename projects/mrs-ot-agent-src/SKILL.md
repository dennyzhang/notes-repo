...
|------------------|---------|
| Fleet SLI/SLO compliance (all PG models) | `bash extract-registry-ids.sh \| bash scan-sli-compliance.sh --slo-ig=10 --slo-threads=3` |
| Single-model SLI check | `echo <entity_id> \| bash tools/scan-sli-compliance.sh` |
...
| [human-input/qe-launch-tuning-playbook.md](human-input/qe-launch-tuning-playbook.md) | QE/prod launch tuning: batch size for traffic spikes (T1), PadCompile relax_ratio (T2), app-layer patching workflow (T3), SDD Lite ↔ SDD tradeoff (T4), test model validation protocol (T5), traffic-correlated example age (T6) |
| [tools/scan-sli-compliance.sh](tools/scan-sli-compliance.sh) | Per-model SLI/SLO compliance checker: snapshot freshness ≤SLO, example age ≤2×SLO, QPS>0. Takes entity IDs from stdin/`--entity-ids=`/`--entity-ids-file=`. PG SLOs: `--slo-ig=10 --slo-threads=3 --slo-feed=30`. JSON output + summary. Wired into `ot-sli-compliance` daily cron. |
| [tools/extract-registry-ids.sh](tools/extract-registry-ids.sh) | Extracts entity IDs from the live PG model registry spreadsheet (all 4 sheets). Pipe into scan-sli-compliance.sh for a full fleet check. |

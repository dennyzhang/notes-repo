...
| Investigating an alert as real when its expected snapshot types don't apply to the model class | Alert says "missing X, Y, Z" but the model has 0 occurrences of X (or Y, or Z) in last 2000+ instances. The alert is misconfigured for this model class — it's a FALSE ALARM, not a real failure. Bot may correctly note "X never produced" as a sub-finding but fail to elevate "alert is misconfigured" to the standing hypothesis. Source: 2026-05-08 m2132070936 — bot drifted to other-model FULL_SNAPSHOT investigation instead of declaring the DENSE_DELTA alert a false alarm. Operator: "you failed to pinpoint the root cause." | **For any alert mentioning specific snapshot types: pull `meta ai.model.instance list --limit=2000` first, group by `snapshot_type`, count occurrences. If ANY expected type has 0 occurrences → standing hypothesis is "alert misconfigured", not "publish broken". Recommend re-tuning the alert's expected-type set.** See R16 in [human-input/triage-discipline.md](human-input/triage-discipline.md). |

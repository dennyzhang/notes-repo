...
> **How a model boots into this flow** (launch → trainer bringup → resolve resume-state →
> load checkpoint → stream/train → publish → loop) + the step-3 resume-pointer/`-1`-anchor
> resolution and its S675238 failure: [retrieval-model-bootstrap-flow.md](./retrieval-model-bootstrap-flow.md).
...
| **`Parent` anchor** | The checkpoint tagged `Parent` = **root of the model's checkpoint lineage**; `-1`/"latest" resume resolves THROUGH it (parent-lineage walk, not max-VALID-version). Distinct from the ubiquitous `is_anchor_checkpoint` tag. Seeded by the **recurring-train parent flow** (`flow_tags=parent`). | tag on one version |
| **Snapshot** | Finalized, ready-to-serve version | snapshot ID |

> **`Checkpoint <eid> : -1 does not exist` after an MRB reset** = the `Parent` lineage root was cleared (VALID children orphaned) → `-1` can't resolve. NOT data loss, NOT a config bug (same `-1` works once the root is back). Fix = re-run the recurring-train **parent** flow to regenerate the `Parent` anchor, then restart the OT job. Full mechanism + confirm-queries + recovery recipe: [retrieval-model-bootstrap-flow.md](./retrieval-model-bootstrap-flow.md) § `-1 does not exist`.

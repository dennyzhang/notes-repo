...
## 2026-06-02 (L78) — Pattern-candidate de-dup (P-number collision fix)

### L78
- **Type:** pipeline bug — concurrent postmortem runs collide on P-numbers.
- **Trigger:** 2026-06-02 daemon-restart catch-up burst. Multiple postmortem digest runs each independently computed "next P = P59" from `known_patterns.md` (max landed = P58) with no reservation/lock → **P59 was stamped on S669486, S668285, AND S667601**; **P60/P61 reused** across S668285 / S667601 / S669147. The proposals are real but un-landable as emitted.
- **Learning:** P-number assignment must be serialized — reserve from a single counter, or (simpler) keep candidates ID-less until land-time and only assign a P-number when promoted to `known_patterns.md`. Until fixed, treat any P-number in a postmortem digest as provisional.
- **Action:** de-duped below into P59–P62 (one per distinct pattern). NONE landed — all are N<3, so they stay candidates per the ≥3-sample invariant. S663166 → no candidate (sparse postmortem, ig_predictor serving-path; validator agreed).

#### Pending Pattern Proposals — de-duped 2026-06-02 (candidates only, N<3, NOT landed)

- **P59 — Accidental OT-job closure during machine cleanup.** Source: S669486 (119.7h, ronghuang). N=1. Symptoms: model age growing, no new snapshots, root OT MAST job NOT running, no infra error. Fix: confirm manual closure (`meta ai.mast-job metadata`), restart via TMS / `mvai online-training-mgr`. Falsifier: MAST attempts show FAILED system error → not P59 (check P17/P38). Validator: confirmed novel.
- **P60 — MVAI base-layer behavioral breaking change breaks newer-app-layer publish (long-running barrier path).** Source: S668285 (72.5h, dafnep). N=1. Symptoms: recurrent publish fails on flows using a newer app layer; prod flow on older layer unaffected; runtime (not build) failure. Fix: `mvai hot-patch -a <app>:<v> -p D106836117 --preserve`; long-term land fix in base layer. Falsifier: both flows on same app-layer version → not P60. Validator: confirmed novel.
- **P61 — IG item-streaming silent death → cache-miss backfill stops.** Source: S667601 + S666044. N=2. Symptoms: streaming job RUNNING but no item-delta updates, cache-miss rate rises ~3%, no MAST error/SJD. Fix: restart streaming job; TMS auto-restart; CUPTI/NCCL deadlock hypothesis (`CUDA_INJECTION64_PATH=none`, unconfirmed). Falsifier: recent SPARSE_DELTA VALID instances → streaming alive. Validator: confirmed; N=2 still <3.
- **P62 — Deprecated upstream Dataswarm table dep + auto-paused writer extends staleness cascade → conveyor blocked.** Source: S669147 (71.2h, ezrak); related S668546. N=1. Distinct from P54 (CDM orchestrator pause, ≥2 pipelines) — this is single-pipeline + deprecated-dep + auto-pause. Fix: remove dep diff (D106848441); unpause via dataswarm maintainer; backfill. Falsifier: CDM orchestrator-level pause → P54.

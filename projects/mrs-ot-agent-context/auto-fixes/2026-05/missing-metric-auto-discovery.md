...
The agent has metrics, thresholds, and detection patterns codified in `auto-learnings/metrics/`, but **no mechanism to proactively detect when a metric is MISSING for a model**. Current gaps discovered this session:
...
- `auto-learnings/metrics/detection-patterns.md` — the patterns this fix operationalizes
- `auto-learnings/metrics/slo-recovery-metrics.md` — the metrics being checked
- `auto-learnings/metrics/queries.md` — the queries used to check them
